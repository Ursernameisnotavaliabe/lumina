...
# ── COMANDOS DO PC ────────────────────────────────────────────────────────────
PC_COMMANDS = {
    'abrir calculadora':  'calc.exe',
    'abrir bloco de notas': 'notepad.exe',
    'abrir explorador':   'explorer.exe',
    'abrir configurações':'ms-settings:',
    'abrir task manager': 'taskmgr.exe',
    'abrir cmd':          'cmd.exe',
}
...
def pc_command():
    ...
    # Comandos diretos
    for key, exe in PC_COMMANDS.items():
        if key in cmd:
            try:
                os.startfile(exe) if exe.startswith('ms-') else subprocess.Popen(exe)
                return jsonify({'ok': True, 'action': f'Abrindo {key}'})
            except Exception as e:
    ...
    if 'volume' in cmd:
        try:
            ...
            if 'mudo' in cmd or 'mute' in cmd:
                ...
                return jsonify({'ok': True, 'action': 'Volume mutado'})
            elif 'desmudo' in cmd or 'unmute' in cmd:
                volume.SetMute(0, None)
                return jsonify({'ok': True, 'action': 'Volume desmutado'})
        except: pass
